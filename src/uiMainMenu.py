...
class uiMainMenu:
    ...
    class janelaCadastro(customtkinter.CTkToplevel):
        def __init__(self, *args, **kwargs):
            ...
            # Texto que apareçe no topo da tela
            self.label = customtkinter.CTkLabel(master=self.frame, text="User Register")
            self.label.pack(pady=12, padx=10)
            ...
            # Opções
            self.opcao_codificar_face = customtkinter.CTkCheckBox(master=self.frame, text="Encode Face")
            self.opcao_codificar_face.pack(pady=12, padx=10)
            self.opcao_codificar_face.select()
            self.opcao_carregar_codificacao = customtkinter.CTkCheckBox(master=self.frame, text="Load Encoding")
            self.opcao_carregar_codificacao.pack(pady=12, padx=10)
            self.opcao_carregar_codificacao.select()

            uiMainMenu._adicionar_botao(self, master=self.frame, text="Register!", command=self.cadastro)
